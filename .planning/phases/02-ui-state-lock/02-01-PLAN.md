...
Note: The orchestrator (useMultiPhaseAutoplay) may need to expose these values. If the props are not currently available, add a TODO comment noting they need to be wired from the single-phase autoplay state. The UI should gracefully handle missing values (show nothing or just "Iteration 1").
  </action>
  <verify>
Run `npm run build` - TypeScript should compile without errors.
