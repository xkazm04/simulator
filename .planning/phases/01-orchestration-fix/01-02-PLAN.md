...
<task type="auto">
  ...
  <action>
1. The hook already creates `orchestratorDeps` (around line 291) but never calls `useAutoplayOrchestrator()`. Add the orchestrator instantiation after the deps are created:
   ```typescript
   // ACTUALLY USE the orchestrator for image generation
   const singlePhaseOrchestrator = useAutoplayOrchestrator(orchestratorDeps);
   ```

2. Add an effect to delegate sketch/gameplay phases to the single-phase orchestrator. Insert after the existing phase transition effect (around line 459):
   ```typescript
   /**
...
3. Add an effect to detect when single-phase completes and advance multi-phase. Insert after the delegation effect:
   ```typescript
   /**
...
4. The return value already has compatible fields. No changes needed there.
...
- Multi-phase test: Start multi-phase autoplay with sketch + gameplay counts
- VERIFY: Both phases progress automatically without user intervention
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: Adjust timeout values for robustness</name>
  <files>app/features/simulator/hooks/useAutoplayOrchestrator.ts</files>
  <action>
1. In the timeout safety net effect (around line 561), increase the timeout from 60 seconds to 120 seconds:
   ```typescript
   const TIMEOUT_MS = 120000; // 120 seconds - generous for slow AI services
   ```

2. Update the log message and error message to reflect the new timeout:
   ```typescript
...
3. In `useMultiPhaseAutoplay.ts`, the phase timeout (around line 605) is already 120 seconds which is reasonable. Leave it as is, but add a comment explaining why:
   ```typescript
...
WHY: The 60-second timeout was too aggressive for some AI image generation services that may take 30-60 seconds per image. 120 seconds provides more headroom while still catching true stalls. The callback-based fix from Plan 01 should make timeouts rare.
  ...
  <verify>
- Grep confirms new timeout value: `grep -n "TIMEOUT_MS.*120000" app/features/simulator/hooks/useAutoplayOrchestrator.ts`
- No timeout errors during normal multi-phase autoplay operation
- If generation genuinely stalls (e.g., network issue), timeout still triggers after 2 minutes
  </verify>
  <done>
...
- [ ] Completion effect advances multi-phase when single-phase finishes
- [ ] Timeout increased to 120 seconds in orchestrator
