...
<task type="auto">
  ...
  <action>
1. Locate the `GenerateOverrides` interface by searching for `export interface GenerateOverrides`. Add a new optional property:
   ```typescript
   onPromptsReady?: (prompts: GeneratedPrompt[]) => void;
   ```

2. Locate the callback invocation points in `handleGenerate` by searching for the following patterns:

   **Success path:** Find `prompts.setGeneratedPrompts(generatedPrompts)` followed by `prompts.pushToHistory(generatedPrompts)`. Add the callback invocation AFTER both calls:
   ```typescript
   ...
   **First fallback path:** Find the block inside `if (result.success) { ... } else { ... }`. In the `else` block, after `prompts.setGeneratedPrompts(fallbackPrompts)` and `prompts.pushToHistory(fallbackPrompts)`, add:
   ```typescript
   overrides?.onPromptsReady?.(fallbackPrompts);
   ```

   **Second fallback path (catch block):** Find the `catch (err)` block. After `prompts.setGeneratedPrompts(fallbackPrompts)` and `prompts.pushToHistory(fallbackPrompts)`, add:
   ```typescript
   overrides?.onPromptsReady?.(fallbackPrompts);
   ```
...
- Run: `grep -n "onPromptsReady" app/features/simulator/SimulatorContext.tsx` - should show interface definition and 3 invocations (success path + 2 fallback paths)
  </verify>
  <done>
- `GenerateOverrides` interface includes `onPromptsReady?: (prompts: GeneratedPrompt[]) => void`
- `handleGenerate` invokes callback in success path and both fallback paths (3 invocations total)
  </done>
</task>
...
<task type="auto">
  ...
  <action>
1. Locate the `AutoplayOrchestratorDeps` interface by searching for `export interface AutoplayOrchestratorDeps`. Update the `onRegeneratePrompts` property signature to accept the callback:
   ```typescript
...
2. Locate the 'generating' case in the main state effect by searching for `case 'generating':`. Find the block that calls `onRegeneratePrompts` (inside the `if (autoplay.state.currentIteration > 1 || generatedPrompts.length === 0)` condition). Replace the existing `onRegeneratePrompts` call with:
   ```typescript
...
3. Locate the backup effect that triggers image generation by searching for the comment `Effect: Trigger image generation when prompts are ready during autoplay`. Add a clarifying comment at the beginning of this effect:
   ```typescript
   /**
